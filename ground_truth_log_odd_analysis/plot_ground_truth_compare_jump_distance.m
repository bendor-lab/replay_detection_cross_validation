function [] = plot_ground_truth_compare_jump_distance(folders,option,method)
...
if exist('log_odd_difference_jump_distance.mat', 'file') ~= 2
            ...
            for nshuffle = 1:length(log_pval{nmethod})
                ...
                parfor threshold = 1:length(p_val_threshold)
                    for nboot = 1:1000 % Bootstrapping 1000 times
                        ...
                        % Events that also pass additional jump distance
    ...
    clear log_odd_difference1 log_odd_difference_CI1 percent_sig_events1 percent_sig_events_CI1 replay_score_compare_CI1 replay_score_compare1...
...
if exist('log_odd_difference_jump_distance_original.mat', 'file') ~= 2
    ...
    save log_odd_difference_jump_distance_original log_odd_difference_original ...
        percent_sig_events_original percent_multi_events_original percent_shuffle_events_original replay_score_original
...
for epoch = 1:3
    subplot(2,2,nfig)
    for nmethod = [4 3 2 1]
        x = mean(log_odd_difference{nmethod}{1}{epoch}(71:end,:),2)'; % from 0.2 to 0.001 rather than 1 to 0.001
        y = mean(percent_sig_events{nmethod}{1}{epoch}(71:end,:),2)';
        ...
        UCI = log_odd_difference_CI{nmethod}{1}{epoch}(71:end,2)';
        UCI(isnan(x)) = [];
        LCI = log_odd_difference_CI{nmethod}{1}{epoch}(71:end,1)';
        LCI(isnan(x)) = [];
        y(isnan(x)) = [];
        x(isnan(x)) = [];
...
xlim([0 2])
set(gca,'xtick',[]);
yticks([0 0.01 0.05 0.5])
cd ground_truth_original\Figure
filename = sprintf('wcorr jump distance false positive rate and p value inset (POST ripple).pdf')
saveas(gcf,filename)
filename = sprintf('wcorr jump distance false positive rate and p value inset (POST ripple).fig')
saveas(gcf,filename)
cd ..
cd ..
clf
...
for nmethod = [4 3 2 1]
    for epoch = 2:3
        ...
        text{count} = sprintf('%s p =< %.3f (proportion = %.3f)',jump_distance{nmethod},p_val_threshold(low_threshold(1)),mean(percent_shuffle_events{nmethod}{epoch}(low_threshold(1),:)));
...
%          [c index(nmethod)] = min(abs(mean(percent_multi_events{nmethod}{1}{epoch},2) - 0.05));
        ...
        patch(error_x,error_y,colour_line{nmethod},'EdgeColor',colour_line{nmethod},'FaceAlpha','0.2','EdgeAlpha','0.2')
        text{count} = sprintf('%s p =< %.3f (proportion = %.3f)',jump_distance{nmethod},p_val_threshold(index(nmethod)),mean(percent_shuffle_events{nmethod}{epoch}(index(nmethod),:)));
