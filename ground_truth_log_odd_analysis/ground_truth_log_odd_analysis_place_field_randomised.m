function [] = ground_truth_log_odd_analysis_place_field_randomised(folders,timebin,BAYSESIAN_NORMALIZED_ACROSS_TRACKS)
% Input:
...
for f = 7
    ...
    %% place field circular shift data
    for shuffle = 1:3
...
%         global_remapped_place_fields_id = [];
        ...
        for event = 1:length(replayEvents_bayesian_spike_count.replay_events)
            % circularly shifting place field  
           circular_shifted_place_fields = [];

            for track_id = 1:2
                circular_shifted_place_fields{track_id} = place_fields_BAYESIAN.track(track_id).raw;

                %circular shift place field bins
                for i = 1 : length(place_fields_BAYESIAN.track(track_id).raw)
                    field =  circular_shifted_place_fields{track_id}{i};
                    circular_shifted_place_fields{track_id}{i} = circshift(field, ceil(rand*length(field)));
                end

                place_fields_BAYESIAN.track(track_id).place_field_shifted{event} = circular_shifted_place_fields{track_id};
...
%                 global_remapped_place_fields_id{event}{track_id}(2,:) = random_cell;
        ...
        circular_shift_original_probability_ratio = [];
        circular_shift_common_good_probability_ratio = [];
%         save extracted_place_fields_BAYESIAN place_fields_BAYESIAN

        % Sequence decoding
        estimated_sequence_place_field_shifted = bayesian_decoding_ground_truth(...
            place_fields_BAYESIAN,'replayEvents_bayesian_spike_count','place_field_shifted',BAYSESIAN_NORMALIZED_ACROSS_TRACKS,'N');
        %                 estimated_sequence_global_remapped = bayesian_decoding(place_fields_BAYESIAN,'replayEvents_bayesian_spike_count','N');

        if ~isfolder({'place_field_shuffles'})
            mkdir('place_field_shuffles')
        end

        cd place_field_shuffles
...
%         save global_remapped_place_fields_id global_remapped_place_fields_id
        ...
        % Save in structure
        for j = 1:length(place_fields_BAYESIAN.track)
            for i = 1:length(estimated_sequence_place_field_shifted(1).replay_events)
                decoded_replay_events(j).replay_events(i).timebins_edges = estimated_sequence_place_field_shifted(j).replay_events(i).replay_time_edges;
                decoded_replay_events(j).replay_events(i).timebins_centre = estimated_sequence_place_field_shifted(j).replay_events(i).replay_time_centered;
                decoded_replay_events(j).replay_events(i).timebins_index = 1:length(estimated_sequence_place_field_shifted(j).replay_events(i).replay_time_centered);
                decoded_replay_events(j).replay_events(i).decoded_position = estimated_sequence_place_field_shifted(j).replay_events(i).replay; % normalized within one track
            end
        end
        ...
        if ~isempty(p)
            for shuffle_id=1:length(shuffle_choice)
                shuffle_type{shuffle_id}.shuffled_track = randomised_data_parallel_shuffles(shuffle_choice{shuffle_id},analysis_type,'place_field_shifted',num_shuffles,...
                    decoded_replay_events,place_fields_BAYESIAN,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
            end
        else
            disp('parallel processing not possible');
            for shuffle_id=1:length(shuffle_choice)
                shuffle_type{shuffle_id}.shuffled_track = randomised_data_run_shuffles(shuffle_choice{shuffle_id},analysis_type,'place_field_shifted',num_shuffles,...
                    decoded_replay_events,place_fields_BAYESIAN,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
            end
        end
        ...
        cd ..
        cd ..
        clear scored_replay decoded_replay_events shuffle_type estimated_sequence_place_field_shifted

        destination = ['place_field_shuffles\shuffle_' num2str(shuffle)];
        ...
        if ~isempty(p)
            for shuffle_id=1:length(shuffle_choice)
                shuffle_type1{shuffle_id}.shuffled_track = randomised_data_parallel_shuffles(shuffle_choice{shuffle_id},analysis_type,'place_field_shifted',num_shuffles,...
                    decoded_replay_events1,place_fields_BAYESIAN,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
                shuffle_type2{shuffle_id}.shuffled_track = randomised_data_parallel_shuffles(shuffle_choice{shuffle_id},analysis_type,'place_field_shifted',num_shuffles,...
            ...
            disp('parallel processing not possible');
            for shuffle_id=1:length(shuffle_choice)
                shuffle_type1{shuffle_id}.shuffled_track = randomised_data_run_shuffles(shuffle_choice{shuffle_id},analysis_type,'place_field_shifted',num_shuffles,...
                    decoded_replay_events1,place_fields_BAYESIAN,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
                shuffle_type2{shuffle_id}.shuffled_track = randomised_data_run_shuffles(shuffle_choice{shuffle_id},analysis_type,'place_field_shifted',num_shuffles,...
                    decoded_replay_events2,place_fields_BAYESIAN,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
        ...
        if isfile('place_field_shifted_original_probability_ratio.mat')
        ...
        estimated_position_place_field_shifted_original = [];
        % Calculate probability ratio (100 shuffles)
        place_cell_index = place_fields_BAYESIAN.good_place_cells;
        bayesian_spike_count = 'replayEvents_bayesian_spike_count';
        estimated_position_place_field_shifted_original = log_odd_bayesian_decoding(place_fields_BAYESIAN,bayesian_spike_count,place_cell_index,timebin,[],[],'place_field_shifted','N');
       
        for event = 1:length(estimated_position_place_field_shifted_original(1).replay_events)
            place_field_shifted_original_probability_ratio{shuffle}{1}(1,event,1) = estimated_position_place_field_shifted_original(1).replay_events(event).probability_ratio;
            place_field_shifted_original_probability_ratio{shuffle}{1}(2,event,1) = 1/(estimated_position_place_field_shifted_original(1).replay_events(event).probability_ratio);
            place_field_shifted_original_probability_ratio{shuffle}{1}(1,event,2) = estimated_position_place_field_shifted_original(1).replay_events(event).probability_ratio_first;
            place_field_shifted_original_probability_ratio{shuffle}{1}(2,event,2) = 1/(estimated_position_place_field_shifted_original(1).replay_events(event).probability_ratio_first);
            place_field_shifted_original_probability_ratio{shuffle}{1}(1,event,3) = estimated_position_place_field_shifted_original(1).replay_events(event).probability_ratio_second;
            place_field_shifted_original_probability_ratio{shuffle}{1}(2,event,3) = 1/(estimated_position_place_field_shifted_original(1).replay_events(event).probability_ratio_second);

            disp(sprintf('place field shifted Event : %i (shuffle %i)...',event,shuffle));
        end
        ...
        parfor nshuffle = 1:1000
            ...
            estimated_position_ratemap_shuffled = log_odd_bayesian_decoding(place_fields_BAYESIAN,bayesian_spike_count,place_cell_index,timebin,[],'ratemap shuffle','place_field_shifted','N');
        ...
        place_field_shifted_original_probability_ratio{shuffle}{2} = ratemap_shuffled_probability_ratio;

        save place_field_shifted_original_probability_ratio place_field_shifted_original_probability_ratio
        clear ratemap_shuffled_probability_ratio place_field_shifted_original_probability_ratio

        % % % Common Good Cells % % %

        if isfile('place_field_shifted_common_good_probability_ratio.mat')
        ...
        % log odd analysis
        estimated_position_place_field_shifted_common_good = [];
        % Calculate probability ratio (100 shuffles)
        estimated_position_place_field_shifted_common_good = log_odd_bayesian_decoding(place_fields_BAYESIAN,bayesian_spike_count,place_cell_index,timebin,[],[],'place_field_shifted','N');
        for event = 1:length(estimated_position_place_field_shifted_common_good(1).replay_events)
            place_field_shifted_common_good_probability_ratio{shuffle}{1}(1,event,1) = estimated_position_place_field_shifted_common_good(1).replay_events(event).probability_ratio;
            place_field_shifted_common_good_probability_ratio{shuffle}{1}(2,event,1) = 1/(estimated_position_place_field_shifted_common_good(1).replay_events(event).probability_ratio);
            place_field_shifted_common_good_probability_ratio{shuffle}{1}(1,event,2) = estimated_position_place_field_shifted_common_good(1).replay_events(event).probability_ratio_first;
            place_field_shifted_common_good_probability_ratio{shuffle}{1}(2,event,2) = 1/(estimated_position_place_field_shifted_common_good(1).replay_events(event).probability_ratio_first);
            place_field_shifted_common_good_probability_ratio{shuffle}{1}(1,event,3) = estimated_position_place_field_shifted_common_good(1).replay_events(event).probability_ratio_second;
            place_field_shifted_common_good_probability_ratio{shuffle}{1}(2,event,3) = 1/(estimated_position_place_field_shifted_common_good(1).replay_events(event).probability_ratio_second);



            disp(sprintf('Common cell place field shifted Event : %i (shuffle %i)...',event,shuffle));
        end
        ...
        place_field_shifted_common_good_probability_ratio{shuffle}{2} = ratemap_shuffled_probability_ratio;

        save place_field_shifted_common_good_probability_ratio place_field_shifted_common_good_probability_ratio

        clear  place_field_shifted_common_good_probability_ratio ratemap_shuffled_probability_ratio place_field_shifted_common_good_probability_ratio
