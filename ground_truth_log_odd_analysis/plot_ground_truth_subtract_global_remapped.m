function [] = plot_ground_truth_subtract_global_remapped(folders,method,option)
...
filename = sprintf('%s (%s) Orignal vs shuffle subtracted.pdf',method{nmethod},option)
sgtitle(filename);
...
for epoch = 1:3
    ...
    for nshuffle = 1:2
        for threshold = 1:length(p_val_threshold)
            ...
            mean_log_odd{nmethod}{epoch}{nshuffle}(threshold) = mean(data{nmethod}{nshuffle}(index{nmethod}{nshuffle}{epoch}(current_index)));
            mean_log_odd{nmethod}{epoch}{nshuffle}(isnan(mean_log_odd{nmethod}{epoch}{nshuffle})) = 0;
            SE_log_odd{nmethod}{epoch}{nshuffle}(threshold) = std(data{nmethod}{nshuffle}(index{nmethod}{nshuffle}{epoch}(current_index)))/sqrt(length((current_index)));
            SE_log_odd{nmethod}{epoch}{nshuffle}(isnan(SE_log_odd{nmethod}{epoch}{nshuffle})) = 0;
...
%         title(Behavioural_epoches{epoch});
    ...
    normalised_log_odd{nmethod}{epoch} = mean_log_odd{nmethod}{epoch}{1}- mean_log_odd{nmethod}{epoch}{2};
    normalised_percent_sig_events{nmethod}{epoch} = percent_sig_events{nmethod}{epoch}{1}- percent_sig_events{nmethod}{epoch}{2};
...
for epoch = 1:3
    ...
    for threshold = 1:length(p_val_threshold)
        for n = 1:3
            ...
            if n == 1
                sc(n) = scatter(mean_log_odd{nmethod}{epoch}{1}(threshold),percent_sig_events{nmethod}{epoch}{1}(threshold),colour_line{n},'filled','MarkerFaceAlpha',alpha_level(threshold))
                
            elseif n == 2
                sc(n) = scatter(normalised_log_odd{nmethod}{epoch}(threshold),normalised_percent_sig_events{nmethod}{epoch}(threshold),colour_line{n},'filled','MarkerFaceAlpha',alpha_level(threshold))

            elseif n == 3
                sc(n) = scatter(mean_log_odd{nmethod}{epoch}{2}(threshold),percent_sig_events{nmethod}{epoch}{2}(threshold),colour_line{n},'filled','MarkerFaceAlpha',alpha_level(threshold))
    ...
    title(Behavioural_epoches{epoch});
    if epoch ==1
        legend([sc(1),sc(2),sc(3)], {'Original','Subtracted from shuffle','Cell ID shuffle'})
    end
end
