function significant_replay_events = number_of_significant_replays_ground_truth(p_value_threshold,ripple_zscore_threshold, method, no_of_shuffles,rexposure_option)
...
for track = 1 :  number_of_tracks % for each track
    ...
    for event = 1 : number_of_events %for each event
        ...
        elseif no_of_shuffles == 2 % 2 shuffles 
            ...
            sig_event_info.p_value(track,event) = min([max(p_values.WHOLE(track,event,2:3)) max(p_values.FIRST_HALF(track,event,2:3)) max(p_values.SECOND_HALF(track,event,2:3))]); %min pvalue between the 3 max pvalues (not the same than the min pvalue)
            
        elseif no_of_shuffles == 1 % 1 shufffle
            % If only POST place bin circular shift
            sig_event_info.significance(track,event) = max(p_values.WHOLE(track,event,3))<p_value_threshold | max(p_values.FIRST_HALF(track,event,3))<p_value_threshold/2 | max(p_values.SECOND_HALF(track,event,1))<p_value_threshold/2; % indices - 0 or 1 for significant event
            sig_event_info.p_value(track,event) = min([max(p_values.WHOLE(track,event,3)) max(p_values.FIRST_HALF(track,event,3)) max(p_values.SECOND_HALF(track,event,3))]); %min pvalue between the 3 max pvalues (not the same than the min pvalue)                      
        end
