function [log_odd] = ground_truth_log_odd_analysis_partial(folders,timebin,posbin,BAYSESIAN_NORMALIZED_ACROSS_TRACKS)
% Input:
...
for f = 1:length(folders)
    ...
    %         probability_ratio_20ms{1}(1,event) = estimated_position_20ms(1).replay_events(event).probability_ratio;
    %         probability_ratio_20ms{1}(2,event) = 1/(estimated_position_20ms(1).replay_events(event).probability_ratio);
    %         disp('20ms decoding...')
    %     end
    %
    ...
    %             ratemap_shuffled_probability_ratio{nshuffle}(1,event) = estimated_position_ratemap_shuffled(1).replay_events(event).probability_ratio;
    %             ratemap_shuffled_probability_ratio{nshuffle}(2,event) = 1/(estimated_position_ratemap_shuffled(1).replay_events(event).probability_ratio);
    %
    %         end
    ...
    %         probability_ratio_common_good{1}(1,event) = estimated_position_common_good(1).replay_events(event).probability_ratio;
    %         probability_ratio_common_good{1}(2,event) = 1/(estimated_position_common_good(1).replay_events(event).probability_ratio);
    ...
    %
    %
    % global remapped 20ms (All good cells)
    
    bayesian_spike_count = 'replayEvents_bayesian_spike_count';
    
    for shuffle = 1:8
        global_remapped_place_fields  = [];
        global_remapped_20ms_probability_ratio = [];
        global_remapped_common_good_probability_ratio = [];
        if isfile('global_remapped_place_fields.mat')
            load global_remapped_place_fields;
        end
        % global remapping by swapping the Cell ID for good cells
        
        %         for track_id = 1:2
        %             place_fields_BAYESIAN.track(track_id).global_remapped_raw = [];
        %             global_remapped_place_fields{shuffle}{track_id} = place_fields_BAYESIAN.track(track_id).raw;
        %
        ...
        %                 global_remapped_place_fields{shuffle}{track_id}{original_cell(j)}=place_fields_BAYESIAN.track(track_id).raw{random_cell(j)};
        %             end
        %
        %             place_fields_BAYESIAN.track(track_id).global_remapped_raw = global_remapped_place_fields{shuffle}{track_id};
        %         end
        ...
        clear global_remapped_place_fields
        
        place_fields_BAYESIAN_global_remapped = [];
        place_fields_BAYESIAN_global_remapped = place_fields_BAYESIAN;
        
        for track_id = 1:2
            place_fields_BAYESIAN_global_remapped.track(track_id).raw = [];
            place_fields_BAYESIAN_global_remapped.track(track_id).raw = place_fields_BAYESIAN.track(track_id).global_remapped_raw;
            
        end
        
        
        
        % Sequenceness decoding
        estimated_sequence_global_remapped = bayesian_decoding_ground_truth(place_fields_BAYESIAN_global_remapped,'replayEvents_bayesian_spike_count',BAYSESIAN_NORMALIZED_ACROSS_TRACKS,'N');
        
        if ~isfolder({'100_global_remapped_shuffles'})
            mkdir('100_global_remapped_shuffles')
        end
        ...
        tempt = replay_scoring([],[0 0 1 0]); % just pacman
        
        % to save new pac man data into the scored replay variable
        load scored_replay
        for track = 1:length(scored_replay)
            for events = 1:length(scored_replay(1).replay_events)
                scored_replay(track).replay_events(events).path_score = tempt(track).replay_events(events).path_score;
            end
        end
        ...
        num_shuffles=1000;
        analysis_type=[0 0 1 0];  %just pacman
        ...
        if ~isempty(p)
            for shuffle_id=1:length(shuffle_choice)
                shuffle_type{shuffle_id}.shuffled_track = ground_truth_parallel_shuffles(shuffle_choice{shuffle_id},analysis_type,num_shuffles,...
                    decoded_replay_events,place_fields_BAYESIAN_global_remapped,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
            end
        else
        ...
        shuffle_folder = sprintf('shuffle_%i',shuffle);
        destination = ['100_global_remapped_shuffles\shuffle_' num2str(shuffle)];
        ...
        cd ..
        cd ..
        tempt1 = replay_scoring(decoded_replay_events1,[0 0 1 0]);
        tempt2 = replay_scoring(decoded_replay_events2,[0 0 1 0]);
        
        for track = 1:length(scored_replay1)
            for events = 1:length(scored_replay1(1).replay_events)
        ...
        if ~isempty(p)
            for shuffle_id=1:length(shuffle_choice)
                shuffle_type1{shuffle_id}.shuffled_track = ground_truth_parallel_shuffles(shuffle_choice{shuffle_id},analysis_type,num_shuffles,...
                    decoded_replay_events1,place_fields_BAYESIAN_global_remapped,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
                shuffle_type2{shuffle_id}.shuffled_track = ground_truth_parallel_shuffles(shuffle_choice{shuffle_id},analysis_type,num_shuffles,...
                    decoded_replay_events2,place_fields_BAYESIAN_global_remapped,BAYSESIAN_NORMALIZED_ACROSS_TRACKS);
        ...
        for type = 1:length(shuffle_choice)
            for track = 1:length(shuffle_type1{type}.shuffled_track)
                for event = 1:length(shuffle_type1{type}.shuffled_track(track).replay_events)
...
%     if isfile('global_remapped_20ms_probability_ratio.mat')
%         load global_remapped_20ms_probability_ratio;
%     end
    %     % % % log odd analysis % % %
    %
    %     % % % All Good Cells % % %
    %     estimated_position_global_remapped_20ms = [];
    %     % Calculate probability ratio (100 shuffles)
    %     estimated_position_global_remapped_20ms = log_odd_bayesian_decoding(place_fields_BAYESIAN,bayesian_spike_count,place_cell_index,timebin,[],'','global remapping','N');
    %     for event = 1:length(estimated_position_global_remapped_20ms(1).replay_events)
    %         global_remapped_20ms_probability_ratio{shuffle}{1}(1,event) = estimated_position_global_remapped_20ms(1).replay_events(event).probability_ratio;
    %         global_remapped_20ms_probability_ratio{shuffle}{1}(2,event) = 1/(estimated_position_global_remapped_20ms(1).replay_events(event).probability_ratio);
    ...
    %     parfor nshuffle = 1:1000
    %         estimated_position_ratemap_shuffled = [];
    %         estimated_position_ratemap_shuffled = log_odd_bayesian_decoding(place_fields_BAYESIAN,bayesian_spike_count,place_cell_index,timebin,[],'ratemap shuffle','global remapping','N');
    %         for event = 1:length(estimated_position_ratemap_shuffled(1).replay_events)
    %             ratemap_shuffled_probability_ratio{nshuffle}(1,event) = estimated_position_ratemap_shuffled(1).replay_events(event).probability_ratio;
    %             ratemap_shuffled_probability_ratio{nshuffle}(2,event) = 1/(estimated_position_ratemap_shuffled(1).replay_events(event).probability_ratio);
    %
    ...
    %     end
    %
    %     global_remapped_20ms_probability_ratio{shuffle}{2} = ratemap_shuffled_probability_ratio;
    %
    %     save global_remapped_20ms_probability_ratio global_remapped_20ms_probability_ratio
    %     clear ratemap_shuffled_probability_ratio global_remapped_20ms_probability_ratio
    %
    %
    %     % % % Common Good Cells % % %
    %
    ...
    %     % Calculate probability ratio (100 shuffles)
    %     estimated_position_global_remapped_common_good = log_odd_bayesian_decoding(place_fields_BAYESIAN,bayesian_spike_count,place_cell_index,timebin,[],'','global remapping','N');
    %     for event = 1:length(estimated_position_global_remapped_common_good(1).replay_events)
    %         global_remapped_common_good_probability_ratio{shuffle}{1}(1,event) = estimated_position_global_remapped_common_good(1).replay_events(event).probability_ratio;
    %         global_remapped_common_good_probability_ratio{shuffle}{1}(2,event) = 1/(estimated_position_global_remapped_common_good(1).replay_events(event).probability_ratio);
    ...
    % %
    % %     save global_remapped_place_fields global_remapped_place_fields
    % %
    % % %     cd 'rate_fixed'
    ...
    % % %     cd ..
    %
    clear estimated_position_global_remapped_20ms global_remapped_20ms_probability_ratio ratemap_shuffled_probability_ratio global_remapped_common_good_probability_ratio
    %
    %
