...
legend([p(5),p(6)], {method_type{5},method_type{6}},'Position',[0.7 0.3 0.05 0.05])
...
cd ground_truth_original\Figure
filename = sprintf('spearman optimisation cell-id shuffled sig proportion.pdf')
saveas(gcf,filename)
filename = sprintf('spearman optimisation cell-id shuffled sig proportion.fig')
saveas(gcf,filename)
cd ..
cd ..
clf
...
set(gca,'xtick',[]);
cd ground_truth_original\Figure
filename = sprintf('spearman optimisation false positive rate and p value inset (POST ripple).pdf')
saveas(gcf,filename)
