function [] = plot_ground_truth_compare_jump_distance_backup(folders,option,method)
...
for epoch = 1:3
    subplot(2,2,nfig)
    for nmethod = [4 3 2 1]
        x = mean(log_odd_difference{nmethod}{1}{epoch}(40:end,:),2)'; % from 0.2 to 0.001 rather than 0.5 to 0.001
        y = mean(percent_sig_events{nmethod}{1}{epoch}(40:end,:),2)';
        ...
        UCI = log_odd_difference_CI{nmethod}{1}{epoch}(40:end,2)';
        UCI(isnan(x)) = [];
        LCI = log_odd_difference_CI{nmethod}{1}{epoch}(40:end,1)';
        LCI(isnan(x)) = [];
        y(isnan(x)) = [];
        x(isnan(x)) = [];
