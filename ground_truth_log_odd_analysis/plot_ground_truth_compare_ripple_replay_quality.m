function [] = plot_ground_truth_compare_ripple_replay_quality(folders,option,method)
...
colour_line = {[8,81,156]/255,[67,147,195]/255,[244,109,67]/255};

% Plot distribution of hippocampal SWR events
fig = figure(2)
...
for epoch = 2:3
    for threshold = 0:1:20
        ...
        s(epoch) = scatter(threshold,proportion_ripple(epoch,threshold+1),'o','filled','MarkerFaceAlpha','0.7','MarkerFaceColor',colour_line{epoch})
        hold on
    end
end
...
xlabel('Ripple Power')
ylabel('Proportion of canditate events')

% Plot distribution of hippocampal replay events (p < 0.05)
subplot(2,2,2)
for epoch = 2:3
    index = epoch_index{1}{1}{epoch};
    sig_index = index(find((log_odd_compare{1}{1}.pvalue(index) <= 0.05)));

    for threshold = 0:1:20
        ripple_pval(epoch,threshold+1) = length(find((log_odd_compare{1}{1}.ripple_peak(sig_index) > threshold) ...
            & log_odd_compare{1}{1}.ripple_peak(sig_index) < threshold+1))...
            / length(find(log_odd_compare{1}{1}.ripple_peak(epoch_index{1}{1}{epoch})));

        s(epoch) = scatter(threshold,ripple_pval(epoch,threshold+1),'o','filled','MarkerFaceAlpha','0.7','MarkerFaceColor',colour_line{epoch})
        hold on
    end
end
legend([s(2),s(3)],{Behavioural_epoches{2},Behavioural_epoches{3}})
...
% Relationship between active place cells (proportion of place cells
% active) and ripple power
active_neuron_SD = [];
active_neuron_mean = [];
subplot(2,2,3)
for epoch = 2:3
    ...
    for threshold = 0:1:20
        ...
        active_neuron{epoch}{threshold+1}(:) = log_odd_compare{1}{1}.neuron_count(find((log_odd_compare{1}{1}.ripple_peak(epoch_index{1}{1}{epoch}) > threshold) ...
            & log_odd_compare{1}{1}.ripple_peak(epoch_index{1}{1}{epoch}) < threshold+1)) ./ place_cell_number(log_odd_compare{1}{1}.experiment(find((log_odd_compare{1}{1}.ripple_peak(epoch_index{1}{1}{epoch}) > threshold) ...
            & log_odd_compare{1}{1}.ripple_peak(epoch_index{1}{1}{epoch}) < threshold+1)));


        active_neuron_SD{epoch}(threshold+1) = std(active_neuron{epoch}{threshold+1});
        active_neuron_mean{epoch}(threshold+1) = mean(active_neuron{epoch}{threshold+1});

        s(epoch) = scatter(threshold,active_neuron_mean{epoch}(threshold+1),'o','filled','MarkerFaceAlpha','0.7','MarkerFaceColor',colour_line{epoch})
        hold on
    end
    %     s(epoch) = errorbar([0:1:20],active_neuron_mean{epoch},active_neuron_SD{epoch},'MarkerFaceColor',colour_line{epoch})

    y = active_neuron_mean{epoch}';
    x = [0:1:20]';
    USD = y + active_neuron_SD{epoch}';
    LSD = y - active_neuron_SD{epoch}';
    patch([x; flipud(x)], [USD; flipud(LSD)], colour_line{epoch},'FaceAlpha','0.1','LineStyle','none')
end
% ylim([0 40])
ax = gca;
ax.FontSize = 12;
...
legend([s(2),s(3)],{Behavioural_epoches{2},Behavioural_epoches{3}})
xlabel('Ripple Power')
ylabel('Mean number of active place cells')
ylabel('Proportion of place cells active')

% Relationship between number of spikes (by place cells) during a canditate event
% active) and ripple power
...
for epoch = 2:3
    ...
    for threshold = 0:1:20
        spike_counts{epoch}{threshold+1}(:) = log_odd_compare{1}{1}.spike_count(find((log_odd_compare{1}{1}.ripple_peak(epoch_index{1}{1}{epoch}) > threshold) ...
            & log_odd_compare{1}{1}.ripple_peak(epoch_index{1}{1}{epoch}) < threshold+1))
        spike_counts_SD{epoch}(threshold+1) = std(spike_counts{epoch}{threshold+1});
        spike_counts_mean{epoch}(threshold+1) = mean(spike_counts{epoch}{threshold+1});

        s(epoch) = scatter(threshold,mean(spike_counts{epoch}{threshold+1}),'o','filled','MarkerFaceAlpha','0.5','MarkerFaceColor',colour_line{epoch})
        hold on
    end
    y = spike_counts_mean{epoch}';
    x = [0:1:20]';
    USD = y + spike_counts_SD{epoch}';
    LSD = y - spike_counts_SD{epoch}';
    patch([x; flipud(x)], [USD; flipud(LSD)], colour_line{epoch},'FaceAlpha','0.1','LineStyle','none')
end
ax = gca;
ax.FontSize = 12;
...
filename = 'relationship between ripple and spike count and active neurons.pdf'
saveas(gcf,filename)
filename = 'relationship between ripple and spike count and active neurons.fig'
saveas(gcf,filename)
cd ..
cd ..
clf
...
cd ground_truth_original
if exist('log_odd_difference_ripple_replay_quality.mat', 'file') ~= 2
...
%                             boot_percent_shuffle_events(threshold,nboot) = ((length(track_1_index)+length(track_2_index))/2) / total_number{nshuffle}(epoch);
    ...
    save log_odd_difference_ripple_replay_quality log_odd_difference log_odd_difference_CI percent_sig_events percent_sig_events_CI...
...
cd ground_truth_original
if exist('log_odd_difference_ripple_replay_quality_original.mat', 'file') ~= 2
    for nmethod = 1:4
        for epoch = 1:3
            for nshuffle = 1:length(log_pval{nmethod})
                ...
                for threshold = 1:length(p_val_threshold)
                    ...
                    boot_percent_sig_events_POST_ripple(threshold) = (length(track_1_index)+length(track_2_index)-multi_event_number)/POST_ripple_total_number{nshuffle}(nmethod,epoch);

                    if nshuffle == 2
                        % cell id shuffled mean significant event proportion
                        boot_percent_shuffle_events(threshold) = ((length(track_1_index)+length(track_2_index))/2) / total_number{nshuffle}(epoch);
                        boot_percent_shuffle_events_POST_ripple(threshold) = ((length(track_1_index)+length(track_2_index))/2) / POST_ripple_total_number{nshuffle}(nmethod,epoch);
                ...
                percent_sig_events_POST_ripple_original{nmethod}{nshuffle}{epoch} = boot_percent_sig_events_POST_ripple(threshold);
                ...
                if nshuffle == 2
                    ...
                    percent_shuffle_events_POST_ripple_original{nmethod}{epoch} = boot_percent_shuffle_events_POST_ripple;
                end
    ...
    save log_odd_difference_ripple_replay_quality_original log_odd_difference_original percent_sig_events_original percent_sig_events_POST_ripple_original...
        percent_multi_events_original percent_shuffle_events_original percent_shuffle_events_POST_ripple_original replay_score_original
    clear boot_log_odd_difference boot_percent_sig_events boot_percent_multi_events boot_replay_score...
        boot_percent_sig_events_POST_ripple boot_percent_shuffle_events boot_percent_shuffle_events_POST_ripple
    cd ..
else
    load log_odd_difference_ripple_replay_quality_original
    cd ..
end
...
cd ground_truth_original\Figure
filename = 'wcorr 2 shuffles ripple replay quality log odds comparision CI (POST ripple threshold).pdf'
saveas(gcf,filename)
filename = 'wcorr 2 shuffles ripple replay quality log odds comparision CI (POST ripple threshold).fig'
saveas(gcf,filename)
cd ..
cd ..
clf
