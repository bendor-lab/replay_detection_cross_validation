function [significant_replay_events sig_event_info] = number_of_significant_replays_ground_truth_backup(p_value_threshold,ripple_zscore_threshold, method, shuffles,rexposure_option)
...
for track = 1 :  number_of_tracks % for each track
    ...
    for event = 1 : number_of_events %for each event
        ...
        elseif strcmp(shuffles,'PRE place and POST time')
            % if Only PRE place field circular shift and POST place bin
            % shift
            sig_event_info.significance(track,event) = max(p_values.WHOLE(track,event,[2 4]))<p_value_threshold | max(p_values.FIRST_HALF(track,event,[2 4]))<p_value_threshold/2 | max(p_values.SECOND_HALF(track,event,[2 4]))<p_value_threshold/2; % indices - 0 or 1 for significant event
            [sig_event_info.p_value(track,event) sig_event_info.segment_id(track,event)] = min([max(p_values.WHOLE(track,event,[2 4])) max(p_values.FIRST_HALF(track,event,[2 4])) max(p_values.SECOND_HALF(track,event,[2 4]))]); %min pvalue between the 3 max pvalues (not the same than the min pvalue)
            %             sig_event_info.significance(track,event) = max(p_values.WHOLE(track,event,2:3) < p_value_threshold); % indices - 0 or 1 for significant event
            %             sig_event_info.p_value(track,event) = max(p_values.WHOLE(track,event,2:3)) ; %min pvalue between the 3 max pvalues (not the same than the min pvalue)
        ...
        elseif strcmp(shuffles,'two POST')
            ...
            % circular shift
            sig_event_info.significance(track,event) = max(p_values.WHOLE(track,event,3:4))<p_value_threshold | max(p_values.FIRST_HALF(track,event,3:4))<p_value_threshold/2 | max(p_values.SECOND_HALF(track,event,3:4))<p_value_threshold/2; % indices - 0 or 1 for significant event
            [sig_event_info.p_value(track,event) sig_event_info.segment_id(track,event)] = min([max(p_values.WHOLE(track,event,3:4)) max(p_values.FIRST_HALF(track,event,3:4)) max(p_values.SECOND_HALF(track,event,3:4))]); %min pvalue between the 3 max pvalues (not the same than the min pvalue)
%             sig_event_info.significance(track,event) = max(p_values.WHOLE(track,event,3:4))<p_value_threshold; % indices - 0 or 1 for significant event
%             sig_event_info.p_value(track,event) = max(p_values.WHOLE(track,event,3:4)); %min pvalue between the 3 max pvalues (not the same than the min pvalue)
