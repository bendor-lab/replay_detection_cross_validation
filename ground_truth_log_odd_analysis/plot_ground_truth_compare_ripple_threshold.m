...
cd ground_truth_original
if exist('log_odd_difference_ripple_effect_original.mat', 'file') ~= 2
    for nmethod = 1:4
        for epoch = 1:3
            for nshuffle = 1:length(log_pval{nmethod})
    ...
    save log_odd_difference_ripple_effect_original log_odd_difference_original percent_sig_events_original percent_sig_events_POST_ripple_original...
        percent_multi_events_original percent_shuffle_events_original percent_shuffle_events_POST_ripple_original
    ...
    load log_odd_difference_ripple_effect_original
    cd ..
end
...
cd ground_truth_original\Figure
filename = 'wcorr 2 shuffles ripple effect log odd comparision CI.pdf'
saveas(gcf,filename)
filename = 'wcorr 2 shuffles ripple effect log odd comparision CI.fig'
saveas(gcf,filename)
cd ..
cd ..
clf
...
% 
% cd ground_truth_original\Figure
% filename = 'wcorr single shuffle ripple effect log odd comparision CI (POST ripple threshold).pdf'
% saveas(gcf,filename)
% cd ..
% cd ..
% clf
...
cd ground_truth_original\Figure
filename = sprintf('wcorr 2 shuffles ripple effect cell-id shuffled sig proportion.pdf')
saveas(gcf,filename)
filename = sprintf('wcorr 2 shuffles ripple effect cell-id shuffled sig proportion.fig')
saveas(gcf,filename)
cd ..
cd ..
clf
