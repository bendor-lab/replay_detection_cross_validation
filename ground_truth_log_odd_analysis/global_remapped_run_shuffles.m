function shuffled_track = global_remapped_run_shuffles(shuffle_choice,analysis_type,num_shuffles,decoded_replay_events,place_fields_BAYESIAN,BAYSESIAN_NORMALIZED_ACROSS_TRACKS)
...
for s=1:num_shuffles
    ...
    elseif strcmp(shuffle_choice, 'PRE spike_train_circular_shift')
        ...
        estimated_position = bayesian_decoding_ground_truth(...
            place_fields_BAYESIAN,shuffled_spike_count,'global_remapped',BAYSESIAN_NORMALIZED_ACROSS_TRACKS,'N');
    ...
    elseif strcmp(shuffle_choice, 'PRE place_field_circular_shift')
        ...
        for event = 1:length(shuffled_place_fields.track(1).global_remapped)

            for track = 1 : num_tracks
                num_cells = size(shuffled_place_fields.track(track).global_remapped{event},2);
                %circular shift place field bins
                for i = 1 : num_cells
                    field =  shuffled_place_fields.track(track).global_remapped{event}{i};
                    shuffled_place_fields.track(track).global_remapped{event}{i} = circshift(field, ceil(rand*length(field)));
                end
            end
        end
%         estimated_position = bayesian_decoding_ground_truth(shuffled_place_fields,'replayEvents_bayesian_spike_count',[],BAYSESIAN_NORMALIZED_ACROSS_TRACKS,'N');
        ...
        estimated_position = bayesian_decoding_ground_truth(...
            shuffled_place_fields,'replayEvents_bayesian_spike_count','global_remapped',BAYSESIAN_NORMALIZED_ACROSS_TRACKS,'N');
