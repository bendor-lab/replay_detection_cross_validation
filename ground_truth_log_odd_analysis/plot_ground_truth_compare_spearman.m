function [] = plot_ground_truth_compare_spearman(folders,option,method)
...
for nmethod = 1:2
    method{nmethod}
    if strcmp(method{nmethod},'spearman')
        cd ground_truth_original
        load log_odd_spearman
        cd ..
    elseif strcmp(method{nmethod},'spearman_all_spikes')
        cd ground_truth_original
        if exist('log_odd_spearman_all_spikes.mat')== 2
            load log_odd_spearman_all_spikes
            cd ..
        else
            cd ..
            [log_odd] = extract_ground_truth_info(folders,'original',method{nmethod},1,3,1.01);
...
cd ground_truth_original
if exist('log_odd_difference_compare_spearman.mat', 'file') ~= 2
    load log_odd_difference_compare_methods
    % copy spearman method
    log_odd_difference1{1} = log_odd_difference{2};
    log_odd_difference_CI1{1} = log_odd_difference_CI{2};
    percent_sig_events1{1} = percent_sig_events{2};
    percent_sig_events_CI1{1} = percent_sig_events_CI{2};
    percent_multi_events1{1} = percent_multi_events{2};
    percent_multi_events_CI1{1} = percent_multi_events_CI{2};

    for epoch = 1:3
        for nmethod = 2:length(method)
            tic
    ...
    save log_odd_difference_compare_spearman log_odd_difference log_odd_difference_CI percent_sig_events percent_multi_events percent_sig_events_CI percent_multi_events_CI
...
cd ground_truth_original
if exist('log_odd_difference_compare_spearman_original.mat', 'file') ~= 2
    for epoch = 1:3
    ...
    save log_odd_difference_compare_spearman_original log_odd_difference_original percent_sig_events_original percent_multi_events_original
    clear boot_log_odd_difference boot_percent_sig_events boot_percent_multi_events
    cd ..
else
    load log_odd_difference_compare_spearman_original
    cd ..
end

method_types = {'Spearman','Spearman all spikes'};
colour_line = {[244,165,130]/255,[128,115,172]/255};
...
legend([sc(1) sc(2)], {method_types{1},method_types{2}})

cd ground_truth_original\Figure
filename = ('spearman median vs all spikes comparision original.pdf')
saveas(gcf,filename)
clf
cd ..
cd ..
...
legend([p(1) p(2)], {method_types{1},method_types{2}})
...
cd ground_truth_original\Figure
filename = sprintf('%s spearman median vs all spikes comparisions CI.pdf',method{nmethod})
saveas(gcf,filename)
cd ..
cd ..
clf
...
for nmethod = 1:2
    ...
    for epoch = 1:3
        ...
        text{nmethod} = sprintf('%s p =< %.3f (proportion = %.3f)',method_types{nmethod},p_val_threshold(index(nmethod)),mean(percent_multi_events{nmethod}{epoch}(index(nmethod),:)));
...
cd ground_truth_original\Figure
filename = sprintf('%s Spearman median vs all spikes multitrack and significant proportion.pdf',method{nmethod})
saveas(gcf,filename)
cd ..
cd ..
clf
...
for epoch = 1:3
    ...
    for nmethod = 1:2
        ...
        text{nmethod} = sprintf('%s p =< %.3f (proportion = %.3f)',method_types{nmethod},p_val_threshold(index(nmethod)),mean(percent_sig_events{nmethod}{epoch}(index(nmethod),:)));
        h(nmethod) = histogram(log_odd_difference{nmethod}{epoch}(index(nmethod),:),nedges,'FaceColor',colour_line{nmethod},'FaceAlpha',0.3,'Normalization','probability');
        box off
        hold on
    end
    ...
    lgd = legend([h(1) h(2)],{text{1},text{2}},'Location','northwest');
    %        lgd = legend([h(1) h(2) h(3) h(4) h(5) h(6)],{text{1},text{2},text{3},text{4},text{5},text{6}},'Location','northwest');
end
titlename = 'Spearman median vs all spikes comparision at equivalent proportion';
sgtitle(titlename)
cd ground_truth_original\Figure
filename = ' Spearman median vs all spikes comparision histogram.pdf';
saveas(gcf,filename)
clf
cd ..
cd ..
...
% method_type = {'Spearman median spike','Spearman all spikes'}';
...
% sgtitle(titlename)
% cd ground_truth_original\Figure
% filename = 'methods comparision 0.05 histogram.pdf';
% saveas(gcf,filename)
% clf
% cd ..
% cd ..

%% Spearman comparision
...
writetable(Table,'spearman compare table.xlsx')
cd ..
cd ..
...
writetable(Table,'spearman compare original table.xlsx')
cd ..
cd ..
