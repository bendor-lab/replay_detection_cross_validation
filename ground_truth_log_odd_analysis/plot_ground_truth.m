function [] = plot_ground_truth(log_odd,option,method)
...
if strcmp(option,'common')
    data = log_odd.common_zscored.original(1,:);
...
for epoch = 1:3
    ...
    ground_truth(epoch).coefficient = b(2);
...
filename = sprintf('%s (%s) log odd vs log pval.pdf',method,option)
saveas(gcf,filename)
cd ..
cd ..
